...
class ActorCritic(object):
	...
	def __init__(self, scope, sess, globalAC = None):
		self.sess = sess
		self.actor_optimizer = tf.train.RMSPropOptimizer(0.001, name = 'RMSPropA')
		...
		if scope == global_net_scope:
			...
			with tf.variable_scope(scope):
				self.state = tf.placeholder(tf.float32, [None, state_shape], name='state')
				self.action_dist = tf.placeholder(tf.float32, [None, action_shape], name = 'action')
				self.target_value = tf.placeholder(tf.float32, [None, 1], name = 'Vtarget')
